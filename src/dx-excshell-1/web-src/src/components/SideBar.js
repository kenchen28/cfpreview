...
function SideBar({ cfpath, variationname, contentfragment }) {
  ...
  const extractImageInfo = () => {
    ...
    keys.forEach((key) => {
      if (
        contentfragment[key] &&
        (contentfragment[key]._authorUrl ||
          contentfragment[key]._publishUrl ||
          contentfragment[key]._dynamicUrl)
      ) {
        cfimagepath =
          process.env.AEM_AUTHOR +
          "/ui#/aem/assetdetails.html" +
          contentfragment[key]._path;
        console.log("Sidebar Asset Source Image Path: ", cfimagepath);
        images.push({
          id: key,
          name: cfimagepath,
          url: contentfragment[key]._authorUrl,
        });
      }
    });
    ...
    <View position="sticky" top="size-0" start="size-0">
      ...
      <View isHidden={!showDetails}>
        ...
        <Heading level={4}>
          ...
          <br />
          {
            contentfragment._metadata.stringMetadata.find(
        ...
        <ActionGroup
          ...
          defaultSelectedKeys={[variationname]}
          marginTop="10px"
          marginBottom="20px"
          onAction={updateVariation}
        >
      ...
      <Button
        width="size-1000"
        aria-label="Content Fragment Admin"
        marginTop="20px"
        marginEnd="10px"
        onPress={() => openInNewTab(cfadminpath)}
      >
        <HomeIcon width="size-200" />
        <Text>Home</Text>
      </Button>
      <Button
        ...
        width="size-1000"
        marginTop="20px"
        aria-label="Edit Content Fragment"
        onPress={() => openInNewTab(cfeditorpath)}
      >
        <EditIcon width="size-200" />
        <Text>Edit</Text>
      </Button>
    </View>
  );
